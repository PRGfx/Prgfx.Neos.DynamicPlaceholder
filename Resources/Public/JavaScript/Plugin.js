 ...
 * @returns {any}
 */
var clientEval = function clientEval(input, node, editorOptions) {
    return eval(input);
};
 ...
 */
var decoratePlaceholderPlugin = function decoratePlaceholderPlugin(plugin, findNode, editorOptions) {
    var originalGetPlaceholder = plugin.prototype.getPlaceholder;
    plugin.prototype.getPlaceholder = function () {
        var originalPlaceholder = originalGetPlaceholder.apply(this);
        if (originalPlaceholder.startsWith('ClientEval:')) {
            if (this.editor && this.editor.neos && this.editor.neos.contextPath) {
                try {
                    var node = findNode(this.editor.neos.contextPath);
                    if (node) {
    ...
    var originalInit = plugin.prototype.init;
    plugin.prototype.init = function () {
        var init = originalInit.bind(this);
...
(0, _neosUiExtensibility2.default)('Prgfx.Neos.DynamicPlaceholder', {}, function (globalRegistry, _ref) {
    var store = _ref.store;

    var ckeRegistry = globalRegistry.get('ckEditor5');
    var ckeConfig = ckeRegistry.get('config');
    var addExistingPlugin = ckeConfig.get('neosPlaceholder');
    if (!addExistingPlugin) return;

    var getNodeByContextPath = function getNodeByContextPath(path) {
        return _neosUiReduxStore.selectors.CR.Nodes.makeGetNodeByContextPathSelector(path)(store.getState());
    };

    var decorateAddedPlugin = function decorateAddedPlugin(ckEditorConfiguration, options) {
        var r = addExistingPlugin(ckEditorConfiguration, options);
        var index = r.plugins.findIndex(function (p) {
            return p.pluginName === 'NeosPlaceholder';
        });
        if (index >= 0) {
